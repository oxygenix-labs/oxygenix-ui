...
export function useDataTable<T = any>(props: UseDataTableProps<T>): UseDataTableReturn<T> {
  const {
    data,
    columns,
    sorting: sortingConfig,
  ...
  // SORTING STATE
  // ========================================
  const [sortState, setSortState] = useState<SortingState>(sortingConfig?.sortState || []);
  ...
  // SELECTION STATE
  // ========================================
  const [selectedRows, setSelectedRows] = useState<string[]>(selectionConfig?.selectedRows || []);
  ...
  const toggleRowSelection = useCallback(
    (rowId: string) => {
      setSelectedRows((prev) => {
        ...
        return prev.includes(rowId) ? prev.filter((id) => id !== rowId) : [...prev, rowId];
      });
    },
  ...
  const selectableRowIds = useMemo(() => selectableRows.map(getRowId), [selectableRows, getRowId]);
  ...
  const toggleSelectAll = useCallback(() => {
    if (isAllSelected) {
      setSelectedRows((prev) => prev.filter((id) => !selectableRowIds.includes(id)));
    } else {
