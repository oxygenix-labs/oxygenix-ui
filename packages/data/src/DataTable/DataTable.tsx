...
export function DataTable<T = any>(props: DataTableProps<T>) {
  ...
  const renderHeader = (column: (typeof columns)[0]) => {
    ...
    const content = typeof column.header === 'function' ? column.header(context) : column.header;
  ...
  const renderCell = (row: T, column: (typeof columns)[0], rowIndex: number) => {
    ...
    return (
      <td
        key={column.id}
        className={cn(styles.cell, column.align && styles[`cell-align-${column.align}`])}
        style={{
          width: column.width,
          minWidth: column.minWidth,
          maxWidth: column.maxWidth,
    ...
    <div className={cn(styles['table-container'], className)} style={style} {...rest}>
      ...
      {slots?.bulkActions && table.selectedRows.length > 0 && (
        <div>
          {typeof slots.bulkActions === 'function'
            ? slots.bulkActions(data.filter((row) => table.selectedRows.includes(getRowId(row))))
            : slots.bulkActions}
        </div>
      )}
      ...
      >
        <thead className={styles.header}>
          <tr role="row">{table.visibleColumns.map(renderHeader)}</tr>
        </thead>
        ...
        <div className={styles['datatable-pagination']}>
          <div className={styles['datatable-pagination-info']}>
            {paginationConfig.showPageInfo && (
              <span>
                Page {table.currentPage} of {table.totalPages} ({table.rows.length} items)
              </span>
            )}
          </div>
