...
export function TableToolbar(props: TableToolbarProps) {
  const { title, selectedCount = 0, leftContent, rightContent, bulkActions, className } = props;
    ...
    <div className={cn(styles['table-toolbar'], className)}>
      <div className={styles['table-toolbar-left']}>
        {hasSelection && bulkActions ? (
          <>
            <span className={styles['table-toolbar-selection-info']}>{selectedCount} selected</span>
            {bulkActions}
          </>
        ) : (
          <>
      ...
      <div className={styles['table-toolbar-right']}>{rightContent}</div>
    </div>
  );
}
